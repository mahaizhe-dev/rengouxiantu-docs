...
--   T10. 旧白名单已废弃，BMConfig.ITEMS 驱动门禁过滤             (BM-S2R 收窄)
...
local function clearModuleCache()
    ...
    -- 注意：config.BlackMerchantConfig 不在此清除 — 它是静态 stub，与 EquipmentData 同级
end
...
-- BM-S2R: BlackMerchantConfig stub（InventorySystem 用 BMConfig.ITEMS 过滤门禁）
package.loaded["config.BlackMerchantConfig"] = {
    ITEMS = {
        wubao_token_box    = { buy_price = 2,  sell_price = 4,  category = "consumable_mat" },
        sha_hai_ling_box   = { buy_price = 2,  sell_price = 4,  category = "consumable_mat" },
        taixu_token_box    = { buy_price = 2,  sell_price = 4,  category = "consumable_mat" },
...
test("T4. 非 BM 消耗品(exp_pill) → 仓库不置脏，门禁不触发 (BM-S2R)", function()
    resetState()
    _mockBackpack[1] = {
    ...
    -- BM-S2R: exp_pill 不在 BMConfig.ITEMS，不触发门禁
    local BMS = require("systems.BlackMarketSyncState")
    local blocked, reason = BMS.IsBlocked()
    assertFalse(blocked, "exp_pill NOT in BMConfig.ITEMS, should NOT trigger gate (BM-S2R)")
    assertEqual(reason, nil, "reason should be nil for non-BM consumable")
    -- 消耗路径不发 save_request
...
test("T10. BM-S2R: 旧白名单已废弃，BMConfig.ITEMS 驱动门禁过滤", function()
    -- _BM_SENSITIVE_CONSUMABLES 保留空表供向后兼容
    local set = InventorySystem._BM_SENSITIVE_CONSUMABLES
    assertTrue(set ~= nil, "set must exist (backward compat)")
    local count = 0
    for _ in pairs(set) do count = count + 1 end
    assertEqual(count, 0, "whitelist should be empty (superseded by BlackMarketSyncState)")

    -- BM-S2R: 非 BMConfig 消耗品不触发门禁
    resetState()
    _mockBackpack[1] = {
    ...
    local BMS = require("systems.BlackMarketSyncState")
    assertFalse(BMS.IsBlocked(), "random_item_xyz NOT in BMConfig.ITEMS, should NOT trigger gate (BM-S2R)")
end)
