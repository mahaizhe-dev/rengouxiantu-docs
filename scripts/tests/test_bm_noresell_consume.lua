...
--
-- 测试方法：构造 mock IS facade + mock manager（背靠本地 slots table），作为参数传入
-- ConsumeConsumable(IS,...) —— 测的就是真实编译进去的 P2 扣除代码，不依赖 stub 生效。
-- 绝不调用 InventorySystem.Init/SetManager（遵守 rules.md 测试安全红线）。
--
-- ⚠️ 副作用规避：真实引擎对 systems 模块走内部模块表，package.loaded 注入对其无效，
-- 故 ConsumeConsumable 尾部会用【真实】EventBus/SyncState。为避免 MarkConsumeUsed
-- 污染玩家真实黑市同步脏标记，本测试统一使用【虚构 consumableId（不在 BMConfig.ITEMS）】，
-- 使尾部 `_BMC.ITEMS[id]` 命中 nil → 不置脏；EventBus 事件因 id 无对应消耗品而无害。
-- ============================================================================

-- 虚构消耗品 ID：绝不能出现在 config.BlackMerchantConfig.ITEMS 中
local DUMMY_ID = "bm_noresell_test_dummy_item"
...
if not okC or not consumables or not consumables.ConsumeConsumable then
    ...
    test("P2-1. 消耗优先扣 bmNoResell（普通堆叠保留）", function()
        local slots = {}
        slots[1] = newStack(DUMMY_ID, 5, false)  -- 普通可卖
        slots[2] = newStack(DUMMY_ID, 5, true)   -- 黑市买入
        local ok = consumables.ConsumeConsumable(makeIS(slots), DUMMY_ID, 3)
        assertTrue(ok, "消耗成功")
    ...
    test("P2-2. 混合消耗(Q4)：需要5，黑市3+普通2 → 全扣完", function()
        local slots = {}
        slots[1] = newStack(DUMMY_ID, 3, true)   -- 黑市3（绑定）
        slots[2] = newStack(DUMMY_ID, 2, false)  -- 普通2（不绑定）
        local ok = consumables.ConsumeConsumable(makeIS(slots), DUMMY_ID, 5)
    ...
    test("P2-2b. 混合消耗：黑市3全扣 + 普通仅扣1", function()
        local slots = {}
        slots[1] = newStack(DUMMY_ID, 3, true)   -- 黑市3
        slots[2] = newStack(DUMMY_ID, 5, false)  -- 普通5
        local ok = consumables.ConsumeConsumable(makeIS(slots), DUMMY_ID, 4)
        assertTrue(ok, "扣4成功")
    ...
    test("P2-3. 数量不足 → 拒绝消耗，堆叠不动", function()
        local slots = {}
        slots[1] = newStack(DUMMY_ID, 2, true)
        local ok = consumables.ConsumeConsumable(makeIS(slots), DUMMY_ID, 5)
    ...
    test("P2-4. 残留临时锁堆叠不参与消耗（旧存档兼容）", function()
        local slots = {}
        slots[1] = newStack(DUMMY_ID, 10, false, true)  -- 旧存档临时锁
        slots[2] = newStack(DUMMY_ID, 3, false)         -- 普通
        -- 可消耗仅普通3，请求5应失败
        local ok = consumables.ConsumeConsumable(makeIS(slots), DUMMY_ID, 5)
        assertFalse(ok, "锁堆不参与，可消耗仅3，请求5失败")
        assertEqual(slots[1].count, 10, "锁堆不动")
        -- 请求3成功，只扣普通
        local ok2 = consumables.ConsumeConsumable(makeIS(slots), DUMMY_ID, 3)
