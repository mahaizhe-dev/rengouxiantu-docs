...
-- BM-FIX-01 + BM-NORESELL(P0.1): 黑市商品级可售逻辑 — 4 条真实路径验证
--
-- 验证目标（BM-FIX-01 §7 + BM-NORESELL P0.1 反转 HOTFIX-SELL-01）：
--   路径1: A锁住，B未锁 → A显示🔒锁定，B显示出售
--   路径2: 顶部全局未同步时 → 卡片级仍显示出售（canSell 只查 L1），
--          但 BM-NORESELL(P0.1): onClick/confirm 在 L2(global_unsync) 时阻断并触发刷盘
--          （反作弊：本地状态未同步不得发起黑市卖出，防陈旧快照写回洗白/退款）
--   路径3: 卡片不会出现"整市场不可售/⏳同步中"（卡片不受 L2 影响）
--   路径4: 锁图标、按钮缺失修复不回退
--
-- 纯 Lua 测试，无引擎依赖。
-- ============================================================================
...
--- 模拟 onClick 中的实时拦截
--- BM-NORESELL(P0.1): 反转 HOTFIX-SELL-01 —— L1 锁定拦截，L2 未同步也拦截（并触发刷盘）
---@param itemId string
---@return string|nil blockReason nil=放行, "locked_item"=L1拦截, "global_unsync"=L2拦截
local function simulateOnClickBlock(itemId)
    ...
    elseif curReason == "global_unsync" then
        -- BM-NORESELL(P0.1): 本地未同步不得发起卖出（防陈旧快照写回），同时触发刷盘
...
--- 模拟确认弹窗最终防线
--- BM-NORESELL(P0.1): L1 锁定拦截，L2 未同步也拦截（最终防线）
---@param itemId string
---@return string|nil blockReason nil=放行, "locked_item"=L1拦截, "global_unsync"=L2拦截
...
test("P2-2: BM-NORESELL(P0.1): L2脏 → onClick 阻断（防陈旧快照写回）", function()
    _lockedItems = {}
    SyncState.ClearAll()
    SyncState.MarkConsumeUsed("some_consume")

    local block = simulateOnClickBlock("herb_002")
    assertEqual(block, "global_unsync", "BM-NORESELL(P0.1): onClick 在 L2 时阻断")
end)

test("P2-3: BM-NORESELL(P0.1): L2脏 → 确认弹窗阻断（最终防线）", function()
    _lockedItems = {}
    SyncState.ClearAll()
    SyncState.MarkWarehouseOp("test_warehouse")

    local block = simulateConfirmBlock("herb_002")
    assertEqual(block, "global_unsync", "BM-NORESELL(P0.1): 确认弹窗在 L2 时阻断")
end)
...
test("P2-4: L2脏 + A锁住 → A显示🔒锁定，B显示出售，两者独立", function()
    ...
    -- BM-NORESELL(P0.1): B 的 onClick 在 L2 时阻断（卡片不变，提交阶段拦截）
    local blockB = simulateOnClickBlock("herb_002")
    assertEqual(blockB, "global_unsync", "BM-NORESELL(P0.1): B的onClick在L2时阻断")
end)

test("P2-5: BM-NORESELL(P0.1): SaveSession脏 → 卡片仍显示出售，onClick阻断", function()
    _lockedItems = {}
    SyncState.ClearAll()
    ...
    local block = simulateOnClickBlock("herb_002")
    assertEqual(block, "global_unsync", "BM-NORESELL(P0.1): SaveSession脏 → onClick阻断")
...
test("P3-4: L1+L2同时存在 → 只有被锁的商品显示🔒，其余仍出售", function()
    ...
    -- BM-NORESELL(P0.1): 卡片不受 L2 影响，但未锁商品的 onClick 在 L2 时阻断
...
test("P4-3: BM-NORESELL(P0.1): onClick L1+L2 均拦截 — L1拦截 → L2阻断 → 全清放行", function()
    -- 场景1: L1 拦截
    _lockedItems = { herb_001 = true }
    SyncState.ClearAll()
    assertEqual(simulateOnClickBlock("herb_001"), "locked_item", "L1拦截")

    -- 场景2: BM-NORESELL(P0.1): L2 阻断（无L1）
    _lockedItems = {}
    SyncState.MarkConsumeUsed("x")
    assertEqual(simulateOnClickBlock("herb_001"), "global_unsync", "BM-NORESELL(P0.1): L2阻断onClick")
...
test("P4-4: BM-NORESELL(P0.1): 确认弹窗 L1+L2 均拦截 — L1拦截 → L2阻断 → 全清放行", function()
    -- 场景1: L1 拦截
    _lockedItems = { herb_001 = true }
    SyncState.ClearAll()
    assertEqual(simulateConfirmBlock("herb_001"), "locked_item", "确认L1拦截")

    -- 场景2: BM-NORESELL(P0.1): L2 阻断
    _lockedItems = {}
    SyncState.MarkWarehouseOp("x")
    assertEqual(simulateConfirmBlock("herb_001"), "global_unsync", "BM-NORESELL(P0.1): L2阻断确认弹窗")
