...
local GM_CATEGORIES = {
    ...
    },
    {
        ...
        commands = {
            ...
            { label = "性能监控开关", action = function()
                local PerfMonitor = require("systems.PerfMonitor")
                local enabled = PerfMonitor.Toggle()
                if enabled then
                    ShowLog("PerfMonitor 已开启 (每10s输出)", {100, 255, 255, 255})
                else
                    ShowLog("PerfMonitor 已关闭", {255, 200, 100, 255})
                end
            end },
            { label = "性能报告查看", action = function()
                local PerfMonitor = require("systems.PerfMonitor")
                local report = PerfMonitor.GetLastReport()
                if report == "" then
                    ShowLog("暂无报告（先开启监控等待10s）", {255, 200, 100, 255})
                else
                    ShowLog(report, {100, 200, 255, 255})
                end
            end },
            { label = "黑市收购", action = function()
                SendGMCommand("bm_recycle", function(msg)
                    ShowLog(msg or "收购完成", {100, 255, 150, 255})
                end)
                ShowLog("收购请求已发送，等待服务端执行...", {255, 200, 100, 255})
            end },
        },
    },
    {
        ...
        commands = {
            { label = "红线+50", action = function()
                local InventorySystem = require("systems.InventorySystem")
                local SaveSystem = require("systems.SaveSystem")
                InventorySystem.AddConsumable("valentine_red_thread", 50)
                ShowLog("同心红线+50, 正在同步...", {255, 200, 100, 255})
                SaveSystem.Save(function(ok)
                    if ok then
                        ShowLog("同步完成！可以开箱了", {100, 255, 100, 255})
                    else
                        ShowLog("同步失败，请稍后重试", {255, 100, 100, 255})
                    end
                end)
            end },
            { label = "玉佩+20", action = function()
                local InventorySystem = require("systems.InventorySystem")
                local SaveSystem = require("systems.SaveSystem")
                InventorySystem.AddConsumable("valentine_pair_jade", 20)
                ShowLog("鸳鸯玉佩+20, 正在同步...", {255, 200, 100, 255})
                SaveSystem.Save(function(ok)
                    if ok then
                        ShowLog("同步完成！可以开箱了", {100, 255, 100, 255})
                    else
            ...
            { label = "双箱各+99", action = function()
                local InventorySystem = require("systems.InventorySystem")
                local SaveSystem = require("systems.SaveSystem")
                InventorySystem.AddConsumable("valentine_red_thread", 99)
                InventorySystem.AddConsumable("valentine_pair_jade", 99)
                ShowLog("红线+玉佩各99, 正在同步...", {255, 200, 100, 255})
                SaveSystem.Save(function(ok)
                    if ok then
                        ShowLog("同步完成！可以开箱了", {100, 255, 100, 255})
                    else
...
function GMConsole.Create(parentOverlay)
    ...
    for _, cat in ipairs(GM_CATEGORIES) do
        ...
        for i = 1, #cat.commands, 2 do
            local leftCmd = cat.commands[i]
            local rightCmd = cat.commands[i + 1]
            local rowChildren = {
                UI.Button {
                    text = leftCmd.label,
                    ...
                    onClick = function() leftCmd.action() end,
                },
            }
            if rightCmd then
