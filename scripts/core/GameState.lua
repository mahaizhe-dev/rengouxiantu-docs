...
GameState.triggeredBattles = {} -- 角色级触发式战斗完成/领奖状态
GameState.accountCosmetics = nil  -- 账号级外观数据 { petAppearances = { [skinId] = { unlocked=true } } }
...
function GameState.GetNearestMonster(x, y, maxRange)
    ...
    for i = 1, #GameState.monsters do
        local m = GameState.monsters[i]
        if m.alive and not m.untargetable then
            local dx, dy = m.x - x, m.y - y
